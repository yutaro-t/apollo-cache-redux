...
import { ApolloReducerConfig, HeuristicFragmentMatcher, ObjectCache, NormalizedCacheObject, StoreReader, StoreWriter, NormalizedCache } from 'apollo-cache-inmemory';
import { reduxNormalizedCacheFactory, ReduxNormalizedCacheConfig } from './reduxNormalizedCache';
...
type Config = Partial<InMemoryCacheConfig & ReduxNormalizedCacheConfig> & Pick<ReduxNormalizedCacheConfig, 'store'>
...
export class ReduxCache extends ApolloCache<NormalizedCacheObject> {
  ...
  protected config: InMemoryCacheConfig & ReduxNormalizedCacheConfig;
  ...
  public write(write: Cache.WriteOptions): void {
    ...
    const newData = produce(this.data, data => {
    ...
    this.data.replace(newData.toObject());
  ...
  public removeOptimistic(idToRemove: string) {
    ...
    if (removedCount > 0) {
      ...
      while (toReapply.length > 0) {
        const innerLayer = toReapply.pop()!;
        this.performTransaction(innerLayer.transaction, innerLayer.optimisticId);
      }
